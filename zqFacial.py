...
def yawn_detection(face, points, frame, starttime):
    ...
    if ratio >= face.Yawn_Radio:  # Yawn
        frame_counter2 += 1
        # cv.putText(frame, str(frame_counter2), (500, 120), cv.FONT_HERSHEY_SIMPLEX, 0.7, [0, 0, 255], 2)
        # face.re_yawn = 'g'
        face.re_yawn = round(frame_counter2 / face.Low_radio_constant2, 2)
    ...
    # ydh
    #face.eye_level = round(1 / (1 + math.exp((face.Closed_Radio_constant_low - frame_counter) / 10)), 2)

    # lzq
    face.re_yawn = round(frame_counter2 / (frame_counter2 +face.Low_radio_constant2) , 2)
    # face.re_yawn>=0.5 疲劳

    # yawn_60s += yawn_10s bug
    # print(time,yawn_10s,yawn_60s)
    if yawn_60s < 1:
        level = 'green'
    if 1 <= yawn_60s < 3:
        level = 'yellow'
    ...
    if time % 10 == 0:
        if (len(yawn_list) > 0):
            if yawn_list[len(yawn_list) - 1][2] != time:
                ...
                with open('D:/desk/drive/biyanhaqianshuju.txt', 'a') as f:
...
def sleepy_detection(face, points, frame, starttime):
    ...
    # lzq
    face.eye_level = round(frame_counter / (frame_counter + face.Closed_Radio_constant_low), 2)
    # face.eye_level>=0.5 疲劳


    # 闭眼判断
    if mean_Ratio < face.Closed_Radio:
        frame_counter += 1  # 闭眼时长
        #face.eye_level = round(frame_counter / face.Closed_Radio_constant_high, 2)
        face.eye_mylevel='g'
    ...
    if time > 1:
        if time % 10 == 0 and is_print1 == False:
            ...
            print('perclos为{}，疲劳等级为{}'.format(perclos, face.eye_mylevel))
            face.perclos = perclos
            # with open('D:/desk/drive/biyanhaqianshuju.txt', 'a') as f:
...
def hough(face, points, frame):  # 暂时没用
...
def gfuncFacial(qQueue, gDict):
    ...
    # 实例化face对象
    face = Face()
    while cap.isOpened() and gDict and gDict['run']:
        if cv.waitKey(100) & 0xFF == ord('q'):
            break
        starttime = datetime.datetime.now()
        # print(datetime.datetime.now()) 每秒3-7帧
        # 帧数计数器
        global eye_captimes
        eye_captimes += 1
        ...
        if ret:
            ...
            for rect in rects:
                ...
                n_time = "\"" + current_time + "\""
                # ydh
                face.perclos = max(face.re_yawn, face.eye_level)

                ret = [zqLib.zqRandom(6),zqLib.zqRandom(6), face.re_yawn
                    , str(face.eye_level), str(face.perclos)]
                zqMySQLdb.sp_mysql("proc_facial_tmp", ret) #写入数据库
                print(n_time, ret)
